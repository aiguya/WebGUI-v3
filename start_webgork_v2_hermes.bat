...
set "APP_URL=http://127.0.0.1:7862"
...
if not exist "%HERMES_EXE%" (
  ...
  echo %HERMES_EXE%
  echo.
  echo Run the Hermes setup in this V2 folder first.
  pause
  exit /b 1
)

echo Checking Hermes xAI OAuth login...
"%HERMES_EXE%" auth status xai-oauth > "%TEMP%\webgork_hermes_auth_status.txt" 2>&1
findstr /i /c:"logged out" "%TEMP%\webgork_hermes_auth_status.txt" >nul 2>nul
if %errorlevel%==0 (
  echo.
  echo xAI OAuth login is required.
  echo Use the Hermes xAI OAuth panel inside WebGork V2 to finish login.
) else (
  echo Hermes xAI OAuth is already logged in.
)

echo.
echo Configuring WebGork Studio V2 to use Hermes Proxy...
