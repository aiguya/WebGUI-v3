...
def grok_official_video_reference_for_path(path):
    ...
    source_parent_post_id = extra.get("official_parent_post_id") or ""
    root_attachment_id = (
        extra.get("official_root_post_id")
        or extra.get("official_root_attachment_id")
        or (source_parent_post_id if source_parent_post_id and source_parent_post_id != generated_id else "")
        or generated_id
        or extra.get("official_post_id")
    )
    source_duration = (
        extra.get("official_output_duration")
        or extra.get("output_duration")
        or extra.get("source_duration")
    ...
    return source_url, {
        ...
        "official_extend_post_id": generated_id or extra.get("official_post_id"),
        "official_parent_post_id": generated_id or extra.get("official_post_id"),
        "official_root_post_id": root_attachment_id,
        "official_root_attachment_id": root_attachment_id,
        "official_source_parent_post_id": source_parent_post_id,
        "official_source_duration": source_duration,
        "official_source_video_ids": extra.get("official_video_ids") or [],
    }


def grok_official_app_chat_video_extend(prompt, source_video, duration=10, resolution="720p", aspect_ratio="2:3", account_id=None):
    account_id = account_id or active_grok_account_id()
    aspect_ratio = official_aspect_ratio(aspect_ratio, fallback="2:3")
    resolution = valid_video_resolution(resolution)
    duration = max(2, min(15, int(duration or 6)))
    ...
    extend_post_id = (
        source_extra.get("official_extend_post_id")
        or official_generated_id_from_url(source_url)
        or source_extra.get("official_parent_post_id")
        or ""
    )
    root_attachment_id = (
        source_extra.get("official_root_attachment_id")
        or source_extra.get("official_root_post_id")
        or source_extra.get("official_source_parent_post_id")
        or extend_post_id
    )
    if not (source_url and extend_post_id):
        raise RuntimeError(
            "Grok official quota video extension requires a library video generated by Grok official home "
            "with an official generated_video URL."
        )
    try:
        video_extension_start_time = video_duration_seconds(source_video)
    except Exception:
        video_extension_start_time = None
    if not video_extension_start_time:
        try:
            video_extension_start_time = float(source_extra.get("official_source_duration") or 0)
        except (TypeError, ValueError):
            video_extension_start_time = 0
    if not video_extension_start_time:
        video_extension_start_time = float(duration)
    video_extension_start_time = round(float(video_extension_start_time), 6)
    clean_prompt = (prompt or "").strip()
    request_body = {
        "temporary": True,
        "modelName": "imagine-video-gen",
        "message": f"{clean_prompt} --mode=custom".strip(),
        "fileAttachments": [root_attachment_id],
        "enableSideBySide": True,
        "responseMetadata": {
            "experiments": [],
            "modelConfigOverride": {
    ...
    reset_grok_official_progress(
        status="running",
        stage="app-chat-video-extend",
        message="Grok official app-chat video extension request sent",
        ...
        source_mode=source_extra.get("official_source_mode") or "",
        official_source_url=source_url,
        official_extend_post_id=extend_post_id,
        official_root_attachment_id=root_attachment_id,
    ...
    browser_response = grok_official_browser_fetch(
        ...
        body=request_body,
        timeout=420,
        target_post_id=extend_post_id,
    )
    status = int(browser_response.get("status") or 0)
    text = browser_response.get("text") or ""
    if status >= 400:
        raise RuntimeError(f"Grok official app-chat video extension request failed: {status} {text[:1200]}")
    ...
    for event in events:
        ...
        update_grok_official_progress(
            status="running",
            stage="app-chat-video-extend",
            message="Grok official app-chat video extension streaming",
            progress=progress,
            event_count=len(events),
            video_url_found=bool(video_urls),
            moderated=moderated[-1] if moderated else False,
    ...
    if not video_urls:
        if moderated and moderated[-1]:
            raise RuntimeError("Grok official app-chat video extension was moderated and no downloadable video URL was returned.")
        raise RuntimeError("Grok official app-chat video extension response did not include a downloadable video URL.")
    ...
    for video_url in reversed(video_urls):
        try:
            ...
            download_errors.append(f"{video_url}: {str(exc)[:500]}")
    if not path:
        raise RuntimeError("Grok official app-chat video extension download failed. " + "; ".join(download_errors[-5:]))
    update_grok_official_progress(
        status="done",
        stage="app-chat-video-extend",
        message="Grok official app-chat video extension completed",
        progress=100,
        official_media_url=used_url,
        official_extend_post_id=extend_post_id,
        official_root_attachment_id=root_attachment_id,
    ...
    return path, {
        ...
        **source_extra,
        ...
        "official_media_url": used_url,
        "official_video_ids": video_ids,
        "official_parent_post_id": extend_post_id,
        "official_extend_post_id": extend_post_id,
        "official_root_attachment_id": root_attachment_id,
        "official_completed": True,
        "official_events": len(events),
        ...
        "official_response_r_rated": r_rated[-1] if r_rated else False,
        "official_conversation_id": conversation_id,
        "official_video_extension_start_time": video_extension_start_time,
        "duration": duration,
        "resolution": resolution,
        "aspect_ratio": aspect_ratio,
        "grok_account_id": account_id,
...
def health():
    ...
    return jsonify({
        ...
        "authenticated": bool(
            ...
            or (cfg["provider"] == "grok_official" and grok_official.get("session_cookie"))
